...
$experience = findAllExperiencesById(isset($_GET['id']) ? (int) $_GET['id'] : 0);
if (!$experience) {
    $_SESSION['messages']['error'] = "expérience n'existe pas";
...
if (
    ...
    if (updateExperience($experience['id'], $domaine_id, $poste, $dated, $datef, $position)) {
        $_SESSION['messages']['sucess'] = "expérience crée avec succés";
        header(("Location: /admin/experiences"));
        exit();
    } else {
                ...
                <div class="form-group">
                    ...
                    <select name="domaine_id" id="domaine_id">
                        ...
                        <?php foreach (findAllDomaines() as $domaine) : ?>
                            <option value="<?= $domaine['id']; ?>" <?= $domaine['id'] == $experience['domaine_id'] ? 'selected' : null; ?>><?= $domaine['nom']; ?></option>
                        <?php endforeach; ?>
                    </select>
                </div>
                <div class="form-group">
                    <label for="poste">Poste:</label>
                    <input type="text" name="poste" id="poste" placeholder="poste" required value="<?= $experience['poste']; ?>">
                </div>
                <div class="form-group">
                    <label for="dated">Date début:</label>
                    <input type="date" name="dated" id="dated" value="<?= $experience['date_debut']; ?>">
                </div>
                <div class="form-group">
                    <label for="datef">Date fin:</label>
                    <input type="date" name="datef" id="datef" value="<?= $experience['date_fin']; ?>">
                </div>
                <div class="form-group">
                    <label for="positinon">Position:</label>
                    <input type="number" name="position" id="position" value="<?= $experience['position']; ?>">
                </div>
                <button type="submit" class="btn btn-light" btn-light>Modifier</button>
            </form>
            <a href="/admin/experiences" class=" btn btn-light">Retour à la liste</a>
        </section>
